...
try:
    ...
    print(
        "Error: jinja2 is required to run tests. Please install it using 'pip install jinja2'"
    )
    sys.exit(1)
    ...
    try:
        ...
        if expected_not_in:
            for n_ex in expected_not_in:
                if n_ex in rendered:
                    print(
                        f"❌ FAILED: Found string that should NOT be present:\n'''{n_ex}'''"
                    )
                    print(f"Rendered:\n{rendered}")
                    success = False
...
def run_prefix_test(name, messages, kwargs=None):
    ...
    for k in range(1, len(messages) + 1):
        ...
        if (
            k < len(messages)
            and messages[k].get("role") == messages[k - 1].get("role")
            and messages[k].get("role") in ("system", "tool")
        ):
            continue
        try:
            current = template.render(
                messages=messages[:k], add_generation_prompt=False, **kwargs
            )
        except Exception:
            print(f"❌ FAILED with exception:\n{traceback.format_exc()}")
            return False
...
def run_oneline_parity_test(name, cases):
    ...
    if (
        not jinja_version
        or not oneline_version
        or jinja_version.group(1) != oneline_version.group(1)
    ):
        print("❌ FAILED: template_version mismatch between jinja and oneline builds.")
        print(
            "Regenerate with: python3 scripts/minify_jinja.py chat_template.jinja chat_template_oneline.txt"
        )
        return False
    ...
    for label, messages, kwargs in cases:
        try:
            ...
            b = oneline_template.render(
                messages=messages, add_generation_prompt=True, **kwargs
            )
        except Exception:
            print(
                f"❌ FAILED with exception on case '{label}':\n{traceback.format_exc()}"
            )
            return False
        if a != b:
...
execute_test(
    ...
    messages=[
        {"role": "user", "content": "Prove Fermat's Last Theorem <|think_xhigh|>"}
    ],
    expected_in=[
        "<|im_start|>system\nReasoning effort is set to xhigh. Please think carefully through the task, validate key assumptions, consider plausible alternatives, and prioritize correctness, consistency, and clarity in the final answer.<|im_end|>\n",
...
execute_test(
    ...
    messages=[
        {
            "role": "user",
            "content": "Quick answer: what is capital of France? <|think_off|>",
        }
    ],
    expected_in=[
        "<|im_start|>user\nQuick answer: what is capital of France?<|im_end|>\n<|im_start|>assistant\n<think>\n\n</think>\n\n"
    ],
    expected_not_in=["<|think_off|>", "Reasoning effort is set to"],
)
...
execute_test(
    ...
    messages=[
        {
            "role": "user",
            "content": [{"type": "text", "text": "Solve this riddle <|think_low|>"}],
        }
    ],
    expected_in=[
        "<|im_start|>system\nReasoning effort is set to low. Keep your thinking brief and focused, moving directly to the conclusion without unnecessary elaboration.<|im_end|>\n",
...
execute_test(
    "16. Inline <|think_xhigh|> in multi-part list[str]",
    messages=[{"role": "user", "content": ["Solve this deeply", "<|think_xhigh|>"]}],
    expected_in=[
        "<|im_start|>system\nReasoning effort is set to xhigh. Please think carefully through the task, validate key assumptions, consider plausible alternatives, and prioritize correctness, consistency, and clarity in the final answer.<|im_end|>\n",
...
execute_test(
    ...
    expected_in=["<|im_start|>user\nHello   world<|im_end|>\n"],
    expected_not_in=["<|think_on|>", "<|think_minimal|>"],
)
...
execute_test(
    ...
    messages=[
        {"role": "user", "content": "Q1"},
        {
            "role": "assistant",
            "content": "Answer 1",
            "reasoning": "Thought via reasoning field",
        },
...
def run_default_effort_knob_test():
    ...
    try:
        ...
        assert src.count("_default_reasoning_effort = 'medium'") == 1, (
            "knob line not found exactly once"
        )
        msgs = [{"role": "user", "content": "hi"}]
        stock = env.from_string(src).render(messages=msgs, add_generation_prompt=True)
        xhigh = env.from_string(
            src.replace(
                "_default_reasoning_effort = 'medium'",
                "_default_reasoning_effort = 'xhigh'",
            )
        ).render(messages=msgs, add_generation_prompt=True)
        off = env.from_string(
            src.replace(
                "_default_reasoning_effort = 'medium'",
        ...
        explicit = env.from_string(
            src.replace(
                "_default_reasoning_effort = 'medium'",
                "_default_reasoning_effort = 'xhigh'",
            )
        ).render(messages=msgs, add_generation_prompt=True, reasoning_effort="low")
        assert "Reasoning effort is set to" not in stock, (
            "stock default must inject no steering line"
        )
        assert "Reasoning effort is set to xhigh" in xhigh, (
            "xhigh knob must inject the xhigh line"
        )
        assert off.endswith("<think>\n\n</think>\n\n"), (
            "none knob must disable thinking"
        )
        assert "Reasoning effort is set to low" in explicit, (
            "an explicit kwarg must still override the knob"
        )
        print("✅ PASSED")
        tests_passed += 1
    except Exception as e:
    ...
    messages=[
        {"role": "user", "content": "Call tool without args"},
        {
            ...
            "tool_calls": [
                {
                    "type": "function",
                    "function": {"name": "no_arg_tool", "arguments": ""},
                }
            ],
        },
    ],
...
execute_test(
            ...
            "tool_calls": [
                {
                    ...
                    "function": {
                        "name": "run_sql",
                        "arguments": {
                            "query": "SELECT * FROM users WHERE id = 1234567890 AND active = true"
                        },
                    },
                }
            ],
    ...
    expected_in=["[TRUNCATED - original length"],
)
...
execute_test(
    ...
    expected_in=["[TRUNCATED - original length 200 chars]"],
)
...
execute_test(
    ...
    messages=[
        ...
        {
            "role": "developer",
            "content": "Mid-conversation update: user changed context.",
        },
...
execute_test(
    ...
    expected_not_in=["<|think_ultracode|>"],
)
    ...
    messages=[
        {"role": "user", "content": "Search for error handlers"},
        {
            ...
            "tool_calls": [
                {
                    "type": "function",
                    "function": {"name": "grep", "arguments": {"pattern": "failed to"}},
                }
            ],
        },
        {
            "role": "tool",
            "content": "src/lib/api.ts:42: throw new Error('failed to fetch');",
        },
        {
            "role": "assistant",
            "content": "",
        ...
        {
            "role": "tool",
            "content": "src/lib/auth.ts:18: throw new Error('failed to authenticate');",
        },
    ],
    expected_not_in=["⚠️ SYSTEM WARNING"],
)
...
execute_test(
    ...
    messages=[
            ...
            "tool_calls": [
                {
                    ...
                    "function": {
                        "name": "grep",
                        "arguments": {"pattern": "console.error"},
                    },
                }
            ],
        },
        {
            "role": "tool",
            "content": "src/utils.js:5: console.error('failed to load config');",
        },
    ],
    expected_not_in=["⚠️ SYSTEM WARNING"],
)
    ...
    messages=[
        {"role": "user", "content": "Run script"},
        {
            ...
            "tool_calls": [
                {
                    "type": "function",
                    "function": {"name": "python", "arguments": {"code": "1/0"}},
                }
            ],
        },
        {
...
execute_test(
    ...
    expected_in=["Reasoning effort is set to low.", "You are terse."],
    expected_not_in=["<|think_low|>"],
)
...
execute_test(
    ...
    expected_in=["Reasoning effort is set to low."],
    expected_not_in=["Reasoning effort is set to xhigh."],
)
...
execute_test(
    ...
    expected_in=["<|im_start|>assistant\n<think>\n\n</think>\n\n"],
    expected_not_in=["Reasoning effort is set to", "<|think_"],
)
...
execute_test("58. Empty message list raises", messages=[], expect_error=True)
...
execute_test(
    ...
    expected_in=["<|vision_start|><|video_pad|><|vision_end|>"],
)
...
execute_test(
    ...
    expected_in=["<function=get_weather>", "<tool_response>\n22C\n</tool_response>"],
)
...
execute_test(
    ...
    expected_in=["<|im_start|>user\nHello<|im_end|>\n"],
    expected_not_in=["<|im_start|>assistant"],
)
...
execute_test(
    ...
    kwargs={"preserve_thinking": False},
    expected_not_in=["plan the fix", "wrap up"],
)
...
execute_test(
    ...
    expected_in=["⚠️ SYSTEM WARNING: The previous tool call returned an error."],
)
...
execute_test(
    ...
    messages=[
        {"role": "user", "content": "Query"},
        {
            ...
            "content": "",
            "tool_calls": [
                {"type": "function", "function": {"name": "db", "arguments": {}}}
            ],
        },
...
execute_test(
    ...
    expected_not_in=["2 consecutive tool errors"],
)
...
agentic_session = [
    ...
    {"role": "user", "content": "Fix the build"},
    {
        ...
        "tool_calls": [
            {
                "type": "function",
                "function": {"name": "run", "arguments": {"cmd": "make"}},
            }
        ],
    },
    {"role": "tool", "content": "Error: missing header"},
    {
        ...
        "tool_calls": [
            {
                "type": "function",
                "function": {"name": "edit", "arguments": {"file": "main.c"}},
            }
        ],
    },
...
execute_test(
    ...
    messages=[
        {"role": "user", "content": "Query"},
        {
            ...
            "content": "",
            "tool_calls": [
                {"type": "function", "function": {"name": "api", "arguments": {}}}
            ],
        },
        {
            "role": "tool",
            "content": '{"error": null, "data": {"rows": [1, 2, 3]}}\n' + "x" * 700,
        },
    ],
    expected_not_in=["⚠️ SYSTEM WARNING"],
)
    ...
    messages=[
        {"role": "user", "content": "Query"},
        {
            ...
            "tool_calls": [
                {
                    "type": "function",
                    "function": {"name": "f", "arguments": {"k": "A" * 40}},
                }
            ],
        },
    ],
...
execute_test(
    ...
    expected_in=["<|vision_start|><|video_pad|><|vision_end|>summarize this video"],
)
...
def run_fuzz_property_test(name, cases, seed):
    ...
    try:
        ...
        failures = fuzz_template.run_fuzz(
            cases=cases,
            seed=seed,
    ...
    if failures:
        first = failures[0]
        print(
            f"❌ FAILED: {len(failures)} invariant violation(s); "
...
print("\n==========================================")
print(
    f"Results: {tests_passed} / {tests_total} tests passed ({tests_passed / tests_total * 100:.1f}%)"
)
print("==========================================")
