...
BASE = "qwen3.8-froggeric-v22.4"  # the upstream release this fork is rebased onto
...
def main() -> int:
    ...
    check(
        "Answer directly and concisely." in full,
        "fast-mode terseness lead present (v22.1.1 split)",
    )
    check(
        'template_version = "qwen3.8-froggeric-v22.4.0"' in full, "version is v22.4.0"
    )
    check("qwen3.8-froggeric-v22.4" in full, "built on the froggeric v22.4 base")
    check(
        "Nail" not in full and "Dagger" not in full,
        "no model-specific identity in template",
    )
    ...
    check(
        "Be a pirate." in render(full, cases["with system prompt"]),
    ...
    check(
        any("PRIORTHOUGHT" in b for b in re.findall(r"<think>(.*?)</think>", mt, re.S)),
    ...
    check(
        STEER not in render(full, cases["no system prompt"]),
        "default (no reasoning_effort): no steering line (upstream medium default)",
    )
    check(
        STEER not in render(full, cases["with system prompt"]),
        "default with system prompt: still no steering line",
    )
    check(
        "Reasoning effort is set to low"
        in render(full, cases["no system prompt"], reasoning_effort="low"),
        "explicit reasoning_effort=low is still honored (opt-in)",
    )
    check(
        MARKER in render(full, cases["no system prompt"], reasoning_effort="low"),
    ...
    check(
        "get_weather" in out and out.count(MARKER) == 1, "tool definitions still render"
    )
    ...
    for fmt in ("xml", "json"):
        fast = render(
            full, weather, tools=tools, enable_thinking=False, tool_call_format=fmt
        )
        ...
        check(
            "think" not in body.lower(),
            f"{fmt}: no thinking reference anywhere in fast prompt",
        )
        check(
            "IMMEDIATELY, with NO conversational" in fast,
            f"{fmt}: tool-call immediacy rule rephrased without 'after thinking'",
        )
        check(
            "IMMEDIATELY after thinking" in slow,
            f"{fmt}: thinking-on keeps the original 'after thinking' wording",
        )
        check(
            "Brief explanation of tool call" in slow,
            f"{fmt}: thinking-on keeps the <think> stanza",
        )
        check(
            "Function calls MUST follow the specified format" in fast,
            f"{fmt}: fast mode still carries the tool-call format rules",
        )
        check(
            "Answer directly and concisely" in fast, f"{fmt}: fast-mode terseness lead"
        )
    ...
    for src_name, src in (("full", full), ("oneline", mini)):
        check(
            MARK in render(src, u),
            f"{src_name}: terse defaults on when the kwarg is absent",
        )
        check(
            MARK in render(src, u, terse=True),
            f"{src_name}: terse=true keeps the block",
        )
        check(
            MARK not in render(src, u, terse=False),
            f"{src_name}: terse=false drops the block",
        )
        check(
            MARK not in render(src, u, terse=False, enable_thinking=False),
            f"{src_name}: terse=false drops it with thinking off too",
        )
        out = render(src, sysu, terse=False)
        check(
            MARK not in out and "You are a pirate." in out,
            f"{src_name}: terse=false leaves the caller's own system prompt intact",
        )
        check(
            "You are a pirate." in render(src, sysu),
    ...
    rt_cases["tool result last"] = (
        [
            ...
            {"role": "tool", "content": "5C"},
        ],
        {"tools": TOOLS},
    )
    rt_cases["assistant prefill"] = (
        [
            {"role": "user", "content": "hi"},
    ...
    for name, case in rt_cases.items():
        ...
        for src_name, src in (("full", full), ("oneline", mini)):
            miss = [
                eff
                for eff, txt in STEER.items()
                if render(src, msgs, reasoning_effort=eff, **extra).count(txt) != 1
            ]
            check(
                not miss,
                f"{src_name}/{name}: effort text present exactly once"
    ...
    for eff, want in ALIASES.items():
        ...
        for src in (full, mini):
            ...
            got.add(
                "xhigh"
                if "set to xhigh" in out
                else "low"
                if "set to low" in out
                else None
            )
        check(
            got == {want},
            f"effort {eff!r} -> {want or 'no instruction'}"
            + (f"  [got {got}]" if got != {want} else ""),
        )

    sysblk = (
        render(mini, cases["no system prompt"])
        .split("<|im_start|>system")[1]
        .split("<|im_end|>")[0]
        .strip()
    )
